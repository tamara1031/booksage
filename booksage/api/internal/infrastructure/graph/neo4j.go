...
func (c *Neo4jClient) InsertNodesAndEdges(ctx context.Context, docID string, nodes []map[string]any, edges []map[string]any) error {
	if len(nodes) == 0 {
		return nil
	}
	...
	// Step 3: Handle Edges (if any provided)
	if len(edges) > 0 {
		edgeQuery := `
			UNWIND $edges AS e
			MATCH (from {node_id: e.from})
			MATCH (to {node_id: e.to})
			MERGE (from)-[r:RELATED_TO]->(to)
			SET r.type = e.type
		`
		_, err = neo4j.ExecuteQuery(ctx, c.driver, edgeQuery,
			map[string]any{"edges": edges},
			neo4j.EagerResultTransformer,
			neo4j.ExecuteQueryWithDatabase(""),
		)
		if err != nil {
			log.Printf("[Neo4j] Warning: edge insertion failed: %v", err)
		}
	}
...
// DeleteDocument deletes all nodes belonging to a document.
func (c *Neo4jClient) DeleteDocument(ctx context.Context, docID string) error {
	query := `
...
func (c *Neo4jClient) DocumentExists(ctx context.Context, docID string) (bool, error) {
...
func (c *Neo4jClient) SearchChunks(ctx context.Context, query string, limit int) ([]domain.SearchResult, error) {
	cypher := `
		MATCH (c:Chunk)
	...
	for _, record := range result.Records {
		...
		out = append(out, domain.SearchResult{
			ID:      "graph-match", // Fixed ID placeholder as Neo4j nodes aren't globally indexed by ID here
			Content: text,
			Score:   0.5,
			Source:  "graph",
		})
