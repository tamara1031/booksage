...
func (c *Client) InsertNodesAndEdges(ctx context.Context, docID string, nodes []map[string]any, edges []map[string]any) error {
	if len(nodes) == 0 {
		return nil
	}
...
func (c *Client) SearchChunks(ctx context.Context, query string, limit int) ([]repository.SearchResult, error) {
	cypher := `
		MATCH (c:Chunk)
