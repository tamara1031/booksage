...
import (
	...
	"github.com/booksage/booksage-api/internal/infrastructure/db"
	neo4jpkg "github.com/booksage/booksage-api/internal/infrastructure/graph"
	"github.com/booksage/booksage-api/internal/infrastructure/llm"
	"github.com/booksage/booksage-api/internal/infrastructure/vector"
	httpport "github.com/booksage/booksage-api/internal/port/http"
...
func (s *Server) Run() error {
	...
	tensorClient := vector.NewInfinityClient(s.cfg.Model.InfinityURL)
	...
	s.dbConn, err = sql.Open(sqliteshim.ShimName, "../data/booksage.db")
	if err != nil {
		return err
	}
	defer func() {
	...
	bunStore, err := db.NewBunStore(s.dbConn, sqlitedialect.New())
	if err != nil {
		return err
	}

	qdrantClient, err := vector.NewQdrantClient(s.cfg.DB.QdrantHost, s.cfg.DB.QdrantPort, s.cfg.DB.QdrantCollection)
	if err != nil {
		return err
	}
	defer func() { _ = qdrantClient.Close() }()

	neo4jClient, err := neo4jpkg.NewNeo4jClient(ctx, s.cfg.DB.Neo4jURI, s.cfg.DB.Neo4jUser, s.cfg.DB.Neo4jPassword)
	if err != nil {
		return err
	}
	...
	sagaOrchestrator := ingest_usecase.NewSagaOrchestrator(qdrantClient, neo4jClient, bunStore, bunStore, localLLMClient, tensorClient)

	// Ingestion Service (Updated to use TensorEngine)
	ingestService := ingest_usecase.NewIngestionService(sagaOrchestrator, tensorClient)

	// Fusion Retriever (Uses Infinity for Tensors)
	fusionRetriever := query_usecase.NewFusionRetriever(qdrantClient, neo4jClient, tensorClient, localLLMClient)

	// Agentic Generator
	generator := query_usecase.NewGenerator(localLLMClient, fusionRetriever)

	// --- Handlers ---

	ingestHandler := httpport.NewIngestHandler(sagaOrchestrator, ingestService, parserClient)
	queryHandler := httpport.NewQueryHandler(generator)

	// ==========================================
	// Initialize and Start HTTP Server
	// ==========================================

	handler := httpport.RegisterRoutes(ingestHandler, queryHandler)
