...
// QdrantClient implements the ingest_usecase.QdrantClient interface using the official Qdrant Go SDK.
type QdrantClient struct {
	client     *pb.Client
	collection string
}

// NewQdrantClient creates a new Qdrant client and ensures the target collection exists.
...
func (c *QdrantClient) createPayloadIndex(ctx context.Context) error {
...
func (c *QdrantClient) InsertChunks(ctx context.Context, docID string, chunks []map[string]any) error {
	if len(chunks) == 0 {
		return nil
	}
...
func (c *QdrantClient) Search(ctx context.Context, queryVector []float32, limit int) ([]domain.SearchResult, error) {
	...
	for _, point := range results {
		text := ""
		// docID := "" // Not needed for ports.SearchResult
		// pageNum := int32(0) // Not needed for ports.SearchResult
...
func (c *QdrantClient) DocumentExists(ctx context.Context, docID string) (bool, error) {
