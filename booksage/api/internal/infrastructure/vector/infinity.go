...
// InfinityClient implements the TensorEngine interface for Infinity.
type InfinityClient struct {
	baseURL    string
	httpClient *http.Client
}

// NewInfinityClient creates a new Infinity client.
...
func (c *InfinityClient) Embed(ctx context.Context, texts []string) ([][]float32, error) {
	if len(texts) == 0 {
		return nil, nil
	}
...
func (c *InfinityClient) Rerank(ctx context.Context, query string, documents []string) ([]float32, error) {
	if len(documents) == 0 {
		return nil, nil
	}
