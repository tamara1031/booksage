...
func (c *Client) InsertChunks(ctx context.Context, docID string, chunks []map[string]any) error {
	if len(chunks) == 0 {
		return nil
	}
...
func (c *Client) Search(ctx context.Context, queryVector []float32, limit int) ([]repository.SearchResult, error) {
	...
	for _, point := range results {
		text := ""
		// docID := "" // Not needed for repository.SearchResult
		// pageNum := int32(0) // Not needed for repository.SearchResult
