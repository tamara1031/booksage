...
func (m *MockQdrantClient) Search(ctx context.Context, vector []float32, limit int) ([]SearchResult, error) {
	return nil, nil
}
...
func (m *MockNeo4jClient) SearchChunks(ctx context.Context, query string, limit int) ([]SearchResult, error) {
	return nil, nil
}
...
func (m *MockDocumentRepository) CreateDocument(ctx context.Context, doc *Document) (int64, error) {
	return 1, nil
}
func (m *MockDocumentRepository) GetDocumentByID(ctx context.Context, id int64) (*Document, error) {
	return &Document{ID: id}, nil
}
func (m *MockDocumentRepository) GetDocumentByHash(ctx context.Context, hash []byte) (*Document, error) {
	// Simulate conflict if hash is specifically marked
	if len(hash) > 0 && hash[0] == 0xF1 { // F1 matches our mock conflict hash
		return &Document{ID: 100, FileHash: hash}, nil
	}
...
func (m *MockSagaRepository) CreateSaga(ctx context.Context, saga *IngestSaga) (int64, error) {
	return 1, nil
}
func (m *MockSagaRepository) GetSagaByID(ctx context.Context, id int64) (*IngestSaga, error) {
	return &IngestSaga{ID: id, Version: 1}, nil
}
func (m *MockSagaRepository) GetLatestSagaByDocumentID(ctx context.Context, docID int64) (*IngestSaga, error) {
	if docID == 100 {
		return &IngestSaga{ID: 100, DocumentID: 100, Status: SagaStatusCompleted, Version: 1}, nil
	}
	return nil, nil
}
func (m *MockSagaRepository) UpdateSagaStatus(ctx context.Context, sagaID int64, currentVersion int, status SagaStatus, currentStep IngestStep, errorMsg string) error {
	return nil
}
func (m *MockSagaRepository) UpsertSagaStep(ctx context.Context, step *SagaStep) (int64, error) {
	return 1, nil
}
func (m *MockSagaRepository) GetSagaSteps(ctx context.Context, sagaID int64) ([]*SagaStep, error) {
	return nil, nil
}
