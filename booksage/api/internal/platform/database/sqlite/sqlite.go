...
func (s *SQLiteStore) CreateDocument(ctx context.Context, doc *ingest.Document) (int64, error) {
	now := time.Now().Unix()
...
func (s *SQLiteStore) GetDocumentByID(ctx context.Context, id int64) (*ingest.Document, error) {
...
func (s *SQLiteStore) GetDocumentByHash(ctx context.Context, hash []byte) (*ingest.Document, error) {
...
func (s *SQLiteStore) CreateSaga(ctx context.Context, saga *ingest.IngestSaga) (int64, error) {
	now := time.Now().Unix()
...
func (s *SQLiteStore) GetLatestSagaByDocumentID(ctx context.Context, docID int64) (*ingest.IngestSaga, error) {
...
func (s *SQLiteStore) UpdateSagaStatus(ctx context.Context, sagaID int64, currentVersion int, status ingest.SagaStatus, currentStep ingest.IngestStep, errorMsg string) error {
	now := time.Now().Unix()
...
func (s *SQLiteStore) UpsertSagaStep(ctx context.Context, step *ingest.SagaStep) (int64, error) {
	now := time.Now().Unix()
	if step.ID == 0 {
		// Insert
...
func (s *SQLiteStore) GetSagaSteps(ctx context.Context, sagaID int64) ([]*ingest.SagaStep, error) {
