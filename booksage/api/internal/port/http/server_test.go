...
func createTestHandler() http.Handler {
	...
	// Services
	saga := ingest_usecase.NewSagaOrchestrator(ingest_usecase.NewMockQdrantClient(), ingest_usecase.NewMockNeo4jClient(), docRepo, sagaRepo, nil, tensor)
	...
	queryHandler := NewQueryHandler(nil) // Generator nil, will panic if used for real logic
