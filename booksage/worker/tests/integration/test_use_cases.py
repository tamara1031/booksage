...
def test_integration_parser_to_domain():
    ...
    metadata = DocumentMetadata(
        book_id="int-test-001", title="Integration Test Book", extra_attributes={"source": "test"}
    )
    assert metadata.book_id == "int-test-001"
...
def test_integration_metadata_inheritance():
    ...
    raw_doc = RawDocument(document_id="123", domain_metadata=meta, elements=[])
